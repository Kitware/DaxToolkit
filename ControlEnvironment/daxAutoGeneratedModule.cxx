...
namespace
{
  daxPort::Types daxStringToType(const std::string& str, int& num_comps)
    {
    ...
    if (regex_match(str, what, ptarray))
      ...
      return daxPort::point_array;
      }
    else if (regex_match(str, what, cellarray))
      {
      num_comps = what.size()==2? atoi(what.str(1).c_str()) : 1;
      return daxPort::cell_array;
      }
    else if (regex_match(str, what, anyarray))
      {
      num_comps = what.size()==2? atoi(what.str(1).c_str()) : 1;
      return daxPort::any_array;
      }
    else if (regex_match(str, what, float_))
      {
      num_comps = what.size()==2? atoi(what.str(1).c_str()) : 1;
      return daxPort::float_;
      }
    num_comps = -1;
    return daxPort::invalid;
    }
}

//-----------------------------------------------------------------------------
daxAutoGeneratedModule::daxAutoGeneratedModule()
{
  this->Type = invalid;
}
...
//-----------------------------------------------------------------------------
const std::string& daxAutoGeneratedModule::GetModuleName() const
{
  return this->ModuleName;
}

//-----------------------------------------------------------------------------
const std::string& daxAutoGeneratedModule::GetFunctorCode() const
{
  return this->FunctorCode;
}

//-----------------------------------------------------------------------------
bool daxAutoGeneratedModule::Initialize(const std::string& definition)
{
  using namespace boost::xpressive;
  ...
  if (what.str(2) == "map_field")
    {
    this->Type = daxModule::map_field;
    }
  else if (what.str(2) == "map_topology_up")
    {
    this->Type = daxModule::map_topology_up;
    }
  else if (what.str(2) == "map_topology_down")
    {
    this->Type = daxModule::map_topology_down;
    }
  ...
  for (size_t cc=0; input_iter != end; ++input_iter, ++cc)
    ...
    int num_comps = 0;
    daxPortPtr port(new daxPort());
    port->SetName(what.str(1));
    port->SetType(daxStringToType(what.str(2), num_comps));
