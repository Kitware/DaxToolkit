...
int UnitTestContTesting(int, char *[])
{
  ...
  if (dax::cont::testing::Testing::Run(TestFail) == 0)
    {
    std::cout << "Did not get expected fail!" << std::endl;
    return 1;
    }
  std::cout << "-------\nThis call should fail." << std::endl;
  if (dax::cont::testing::Testing::Run(BadTestAssert) == 0)
    {
    std::cout << "Did not get expected fail!" << std::endl;
    return 1;
    }
...
//DAX_ASSERT_CONT only is a valid call when you are building with debug
  ...
  if (dax::cont::testing::Testing::Run(BadAssert) == expectedResult)
    {
    std::cout << "Did not get expected fail!" << std::endl;
    return 1;
    }
  ...
  return dax::cont::testing::Testing::Run(GoodAssert);
}
